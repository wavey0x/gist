...
test("the static offline shell has syntax-valid external assets", () => {
  ...
  assert.match(shellHtml, /href="\/offline-shell\.css\?v=2"/);
  assert.match(shellHtml, /src="\/offline-shell\.js\?v=2"/);
...
test("install precaches the complete, version-matched shell", async () => {
  ...
  assert.deepEqual(harness.addedAssets, [
    "/offline-shell.html?v=2",
    "/offline-shell.css?v=2",
    "/offline-shell.js?v=2",
    "/icons/icon-192.png",
    "/icons/icon-512.png"
  ]);
...
test("offline gist navigation falls back to the cached shell", async () => {
  ...
  assert.equal(await response.text(), "cached:/offline-shell.html?v=2");
});
